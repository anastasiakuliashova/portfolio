...
var meats= pizzaIngredients.meats;
...
var lenOfMeats = meats.length;
var nonMeats= pizzaIngredients.nonMeats;
nonMeats = [
  "White Onions",
  "Red Onions",
  "Sauteed Onions",
...
var lenOfnonMeats = nonMeats.length;
var cheeses= pizzaIngredients.cheeses;
cheeses = [
  "American Cheese",
  "Swiss Cheese",
  "Goat Cheese",
...
var lenOfCheeses = cheeses.length;
var sauces=pizzaIngredients.sauces;
sauces = [
  "Red Sauce",
  "Marinara",
  "BBQ Sauce",
...
var lenOfSauces = sauces.length;
var crusts=pizzaIngredients.crusts;
crusts = [
  "White Crust",
  "Whole Wheat Crust",
  "Flatbread Crust",
...
var lenOfAgj = adjectives.length;  
var lenOfNouns = nouns.length;

// Generates random numbers for getAdj and getNoun functions and returns a new pizza name
function generator(adj, noun) {
  var adjectives = getAdj(adj);
  var nouns = getNoun(noun);
  var randomAdjective = parseInt(Math.random()*lenOfAgj);
  var randomNoun = parseInt(Math.random() * lenOfNouns);
  var name = "The " + adjectives[randomAdjective].capitalize() + " " + nouns[randomNoun].capitalize();
  return name;
}

function randName(len){
	var res =  parseInt(Math.random() * len);
	return  res;
}
// Chooses random adjective and random noun
function randomName() {
  //var randomNumberNoun = parseInt(Math.random() * lenOfNouns);
  var randomNumberAdj = randName(lenOfAgj);
  var randomNumberNoun = randName(lenOfNouns);
  return generator(adjectives[randomNumberAdj], nouns[randomNumberNoun]);
}

function randIngred(arr, len){
	var ret = arr[Math.floor(Math.random() * len)];
	return ret;
}
...
var makeRandomPizza = function() {
  ...
  for (var i = 0; i < numberOfMeats; i++) {
    pizza = pizza + ingredientItemizer(randIngred(meats, lenOfMeats));
  }

  for (var j = 0; j < numberOfNonMeats; j++) {
    pizza = pizza + ingredientItemizer(randIngred(nonMeats, lenOfnonMeats));
  }

  for (var k = 0; k < numberOfCheeses; k++) {
    pizza = pizza + ingredientItemizer(randIngred(cheeses, lenOfCheeses));
  }

  pizza = pizza + ingredientItemizer(randIngred(sauces, lenOfSauces));
  pizza = pizza + ingredientItemizer(randIngred(crusts, lenOfCrusts));
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
	  var pzzSize = document.querySelector("#pizzaSize").innerHTML;
    switch(size) {
      case "1":
        pzzSize = "Small";
        return;
...
var pzzQuery=document.querySelectorAll(".randomPizzaContainer");
  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    for (var i = 0; i < pzzQuery.length; i++) {
      var dx = determineDx(pzzQuery[i], size);
      var newwidth = (pzzQuery[i].offsetWidth + dx) + 'px';
      pzzQuery[i].style.width = newwidth;
    }
  }
